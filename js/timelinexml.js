(function($) {
	...
	function Model(options) {
		...
		this.htmlPriorityClassName = options.htmlPriorityClassName;
	...
	Model.prototype.getHTMLContent = function() {
		...
		for (var i=0; i<events.length; i++) {
			this.rawData[i] = {
				...
				priority : 	$(events[i]).find('.'+this.htmlPriorityClassName).html(),
	...
	Model.prototype.parseRawData = function() {
		...
		for (var i=0; i<root.rawData.length; i++) {
			if (root.rawData[i].date.search('BC') == -1) {
				...
				var newDate = moment(root.rawData[i].date);
				...
				var datestring = moment(newDate).format('YYYY-MM-DD HH:mm');
			} else {
				var datestring = root.rawData[i].date;
				year = parseInt('-'+root.rawData[i].date.replace(' ', '').replace('BC', ''));
			}
			
			var priority = root.rawData[i].priority;
	...
	View.prototype.addMonth = function() {
		...
		if (root.monthId%12 != 0 && root.showMonths) {
			...
			$('#timeline-month-'+root.monthId).css({ "left" : methods.lim(root.monthPosition, 0, root.width)/root.width*100+'%' });
		}
	...
	View.prototype.addYear = function() {
		...
		if (root.showYears && root.yearId%root.showEveryNthYear == 0) {
			...
			$('#timeline-year-'+root.yearId).css({ "left" : methods.lim(root.yearPosition, 0, root.width-1)/root.width*100+'%' });
		}
	...
	View.prototype.positionEvents = function() {
		...
		for (i=0; i<model.events.length; i++) {
			$('#timeline-event-'+model.events[i].id).css({ "left" : model.events[i].position/root.width*100+'%', "z-index" : i });
		}
	},
	...
	View.prototype.selectEvent = function(target) {
		...
		$("html, body").animate({scrollTop: $('.timeline-wrap').offset().top}, 100);							
		});
	},
 	...
 	$.fn.extend({ 
		timelinexml : function(options) {
			var settings = $.extend({
				...
				htmlPriorityClassName : "timeline-priority",
