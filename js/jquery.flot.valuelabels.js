    	    ...
    	    $.each(plot.getData(), function(ii, series) {
    		    ...
    		    for (var i in series.data) {
        			...
        			if (val!=last_val || i==series.data.length-1) {
        				var xx = series.xaxis.p2c(x)+plot.getPlotOffset().left/1.5+2;
        				var yy = (plot.height()+series.yaxis.p2c(y))*0.5;
                        var yHeigth = plot.height()-series.yaxis.p2c(y);
